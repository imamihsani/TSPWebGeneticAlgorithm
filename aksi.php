...
if ($act == 'login') {
    ...
    if ($row) {
        ...
        print_msg("Username atau password salah.");
    }

    /** KURIR*/
    $row = $db->get_row("SELECT * FROM tb_kurir WHERE user='$user' AND pass='$pass'");
    if ($row) {
        $_SESSION['login'] = $row->user;
        redirect_js("index_kurir.php");
    } else {
        print_msg("Username atau password salah.");
    }
...
if ($mod == 'titik_tambah') {
    ...
    if ($kode_kelompok == '' || $kode == '' || $nama == '' || $lat == '' || $lng == '' || $penerima =='' || $notelp =='')
        print_msg("Field bertanda * tidak boleh kosong!");
    elseif ($db->get_results("SELECT * FROM tb_titik WHERE kode_titik='$kode'"))
        print_msg("Kode sudah ada!");
    else {
        $db->query("INSERT INTO tb_titik (kode_kelompok, kode_titik, nama_titik, lat, lng, penerima, notelp) 
            VALUES ('$kode_kelompok', '$kode', '$nama', '$lat', '$lng', '$penerima', '$notelp')");
    ...
    if ($kode_kelompok == '' || $kode == '' || $nama == '' || $lat == '' || $lng == '' || $penerima =='' || $notelp =='')
        ...
        $db->query("UPDATE tb_titik SET kode_kelompok='$kode_kelompok', kode_titik='$kode', nama_titik='$nama', lat='$lat', lng='$lng', penerima='$penerima', notelp='$notelp' WHERE kode_titik='$_GET[ID]'");
        redirect_js("index.php?m=titik");
    }
...
if ($mod == 'titik_tambah') {
    ...
    elseif ($db->get_results("SELECT * FROM tb_titik WHERE kode_titik='$kode'"))
        ...
        $db->query("INSERT INTO tb_bobot(ID1, ID2, bobot) SELECT kode_titik, '$kode', 0  FROM tb_titik");
        $db->query("INSERT INTO tb_bobot(ID1, ID2, bobot) SELECT '$kode', kode_titik, 0  FROM tb_titik WHERE kode_titik<>'$kode'");
        redirect_js("index.php?m=titik_kurir");
    }
} else if ($mod == 'titik_ubah') {
    $kode_kelompok = $_POST['kode_kelompok'];
    $kode = $_POST['kode'];
    ...
    $db->query("DELETE FROM tb_titik WHERE kode_titik='$_GET[ID]'");
    $db->query("DELETE FROM tb_bobot WHERE ID1='$_GET[ID]' OR ID2='$_GET[ID]'");
    header("location:index.php?m=titik_kurir");
}
